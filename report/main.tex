...
\subsection{Ambiente de Simulação e Topologia}

As simulações foram executadas utilizando a ferramenta \texttt{p4app},
a qual automatiza o processo de compilação dos programas P4 e gerencia a criação
de uma infraestrutura de rede virtualizada via emulador Mininet.
Os comutadores de software operam com base no modelo comportamental \textit{Behavioral Model v2} (BMv2).
A topologia de rede adotada é composta por um hospedeiro de origem,
conectado a um hospedeiro de destino por meio do switch programável executando o datapath P4.

Para fins de validação e análise estatística, cada cenário experimental foi executado $R = 5$ vezes de forma independente.
Em cada rodada de simulação, o tráfego é composto por 100 fluxos concorrentes baseados no protocolo TCP,
distribuídos em 90 fluxos legítimos (gerando tráfego com o mesmo endereço IP de origem, mas com portas de origem diferentes);
e 10 fluxos falsificados (gerando tráfego de ataque com endereços IP de origem forjados).

O tráfego de carga útil é do tipo \texttt{bulksend},
de modo a simular transmissões massivas e contínuas de pacotes de dados.
A duração total de cada execução de simulação foi definida em 100 segundos.
Para avaliar a reação dinâmica da heurística no switch,
os fluxos legítimos operam de forma contínua durante toda a simulação,
enquanto o ataque de falsificação é iniciado no segundo 30 e encerrado no segundo 90.

Os fluxos de ataque (falsificados) são gerados com o intuito de tentar burlar a validação do P4Drop.
Para tanto, o atacante adota uma probabilidade fixa de 10\% para simular retransmissões de pacotes TCP,
visando convencer o comutador de que se trata de um fluxo legítimo operando em malha fechada.
Para avaliar a robustez do switch contra diferentes sofisticações de ataque,
foram implementados dois mecanismos distintos para essa simulação de retransmissão:
\begin{itemize}
    \item \textbf{Modelo Sem Histórico (\textit{No-Backlog}):} O atacante simula as retransmissões
    repetindo estritamente o número de sequência do último pacote que foi enviado.
    \item \textbf{Modelo Com Histórico (\textit{Backlog}):} O atacante mantém uma fila de histórico
    com capacidade para armazenar os últimos 32 números de sequência transmitidos.
    A simulação de retransmissão é executada selecionando e repetindo um número de sequência
    aleatório contido nesse histórico.
\end{itemize}

\subsection{Métricas de Desempenho}

Para analisar o comportamento do P4Drop e do P4DropExt,
foram avaliadas duas métricas fundamentais:
...
\begin{enumerate}
    ...
    \end{itemize}
    As taxas de LDR e ALR são calculadas pelas Equações~(\ref{eq:eval:ldr}) e~(\ref{eq:eval:alr}):
    \begin{equation}
    ...
    Para representar a variabilidade dessas taxas ao longo das rodadas, os resultados são reportados
    com barras de erro correspondentes ao Erro Padrão da Média (SEM - \textit{Standard Error of the Mean}), dado por:
    \begin{equation}
        SEM = \frac{S}{\sqrt{R}}
        \label{eq:eval:sem}
    \end{equation}
    em que $S$ representa o desvio padrão amostral das taxas LDR e ALR, e $R = 5$ é o número de rodadas.

    \item \textbf{Probabilidade de Entrega por Índice de Pacote:} Permite analisar a velocidade de reação
    da heurística no plano de dados independente do fator tempo.
    Para o conjunto consolidado de fluxos de ataque obtido ao longo de todas as rodadas,
    representando uma população total de $M = R \times F = 50$ fluxos maliciosos,
    define-se para cada fluxo $i \in \{1, 2, \dots, M\}$ um perfil de entrega
    $P_i = [p_{i,1}, p_{i,2}, \dots, p_{i,K}]$, no qual $p_{i,k} = 1$ caso o $k$-ésimo pacote cronológico
    daquele fluxo tenha sido encaminhado com sucesso, e $p_{i,k} = 0$ se tiver sido descartado pelo switch.
    A probabilidade de entrega para o $k$-ésimo pacote, $P(k)$, é computada pela média:
    \begin{equation}
        P(k) = \frac{1}{M} \sum_{i=1}^{M} p_{i,k}
        \label{eq:eval:profile}
    \end{equation}
    Essa métrica indica quantos pacotes de ataque conseguem vazar e atravessar o comutador
    antes que o fluxo seja definitivamente bloqueado (isto é, até que a probabilidade $P(k)$ caia para a taxa residual de amostragem de 5\%).
\end{enumerate}
